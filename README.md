...
Callsense is a React/Vite application that provides AI-powered analysis of sales calls.  It allows users (sales agents and administrators) to upload audio recordings or PDF documents and receive detailed call transcripts, performance metrics, and coaching feedback.  A **live copilot** feature uses Google’s Gemini AI (via the @google/genai SDK) to provide real-time suggestions during calls.  Target users include sales teams and managers: *Responders* (sales agents) who make calls, and *Admins* who oversee call quality and knowledge assets.  The app’s key features include file upload (audio calls and company knowledge PDFs), automated call analysis (transcription, sentiment, compliance, and summary), interactive visualizations (charts, tables), and a real-time AI assistant.  The business value lies in **improving sales performance** through data-driven insights and AI guidance. 

- **Project purpose:** Automate and augment sales call evaluation and coaching using AI.
- **Target users:** Sales representatives (“Responders”) and their managers (“Admins”).
- **Core value:** Real-time transcription and feedback improve agent effectiveness and customer engagement.

The repository consists of a single-page app with no separate backend (aside from Google Cloud functions).  It uses Firebase for authentication and data storage, and Google’s Generative AI APIs (Gemini models) for analysis.  Below is a detailed breakdown of the system.
...
```mermaid
erDiagram
  USERS ||--o{ CALL_ANALYSIS : creates
  USERS ||--o{ KNOWLEDGE_BASES : uploads
  USERS ||--o{ RESPONDER_FEEDBACK : writes
  CALL_ANALYSIS {
    string id PK
    string agentId FK
    date createdAt
    number conversionScore
    string sentimentClient
    string sentimentResponder
    boolean dmVerified
    boolean needDiscovery
    boolean priceAdherence
    boolean objectionHandled
    boolean hardCloseAttempted
  }
  USERS {
    string id PK
    string email
...
- **Firestore Rules:** Rules begin with `rules_version = '2'` and include checks like:
  ...
  ...
  ```
  For instance, only Admins can delete any call report, while Responders can delete *their own* report only if its `conversionScore < 10`. This demonstrates careful access control.
  <br>**Firebase Guidance:** Official docs state that building **user-based/role-based systems requires Firebase Auth + Firestore Rules**. The app’s rules follow this approach, binding `request.auth.uid` to data documents (e.g. verifying `resource.data.agentId == request.auth.uid`).
...
**Recommendation:** Remove hard-coded keys, enforce least privilege. For the Live API, use **ephemeral token** authentication instead of embedding the API key to reduce exposure. 
...
- **WebSockets:** The live copilot uses streaming WebSockets (AudioContext + Google Live API). This is efficient for real-time but can strain bandwidth. The app converts audio to 16-bit PCM, 16kHz (documented in [19]) to meet Gemini’s spec. This keeps payload minimal. We should ensure the browser’s audio context handles 16000 Hz conversion correctly (the code uses `audioContext.sampleRate`).
...
- **Gemini Content Models:** The `@google/genai` SDK is used to call Gemini-2.5-flash models (text + base64 attachments). It generates structured JSON output by specifying `responseSchema`. Structured output is emphasized in Google’s docs as enabling “predictable, type-safe results” from unstructured input.
...
- **Live AI Copilot:** For real-time calls, the app uses the Gemini **Live API** (streaming). The `liveService` sets up an audio WebSocket session. It sends audio frames as they’re captured, and receives text “suggestions” via server messages. The system message instructs Gemini to act as a sales copilot (see code: “Your goal: Help the Agent convert the lead. Listen for questions about pricing, features or objections…”). The live model `gemini-2.5-flash-native-audio-preview-12-2025` is used. The technical spec indicates the model accepts raw PCM audio at 16kHz. The Live API returns interim transcripts (input transcription) and final suggestions (output transcription). When the model’s turn completes, the code emits `onSuggestion` for the UI to display.
...
2. **Setup:** Run `npm install` to fetch dependencies. 
3. **Environment Variables:** Create `.env.local` with required keys:  
   - `VITE_FIREBASE_API_KEY`, `VITE_FIREBASE_AUTH_DOMAIN`, etc. (for Firebase, if overriding defaults)  
   - `VITE_GEMINI_API_KEY` (Gemini API key from Google Cloud). The README specifies `GEMINI_API_KEY` in `.env.local`.
4. **Run Locally:** `npm run dev` starts the Vite dev server (usually at `localhost:3000`).
5. **Build:** `npm run build` produces static assets for production.
6. **Deployment:** Deploy to any static host (e.g. Vercel, Firebase Hosting). If using Vercel, the `/api/gemini.ts` will be deployed as a serverless function automatically. Firebase Hosting can also serve static files but would require a Cloud Function for the Gemini endpoint.

No Dockerfile or CI config is provided. We assume standard deployment. The frontend is decoupled from any own backend aside from the serverless Gemini function and Firebase. 

## Testing

- **Existing Tests:** *None*. No `*.test.tsx` or similar present.
- **Suggested Test Cases:** 
  - *Auth flows:* test signup, login, signup with invalid email, unverified user access.
  - *File uploads:* uploading valid/invalid PDF and audio, UI handling of large files.
  - *AI integration:* mock Gemini responses to test parsing logic (especially with the `responseSchema`). 
  - *Firestore rules:* use Firebase emulator to test that rules enforce admin vs responder rights.
  - *Edge cases:* nil responses, long transcripts, API timeouts. 
  - *UI components:* ensure Dashboard and report view render correctly with dummy data, accessibility checks.
- **Test Coverage:** Aim for high coverage of the service logic. For example, unit-test `geminiService` functions by stubbing `GoogleGenAI` to return known JSON and verifying output formatting.

## Recommended Improvements and Roadmap

- **Security:** Use ephemeral auth tokens for the Live API and Gemini calls as recommended. Remove hardcoded keys and strictly use `.env` variables.
- **Modularize AI code:** Refactor `analyzeAudioCall` into smaller functions (e.g. separate transcript extraction and audit).
- **UI/UX:** Add loading spinners and error messages around AI calls. Allow pagination/filtering of call reports. Add user profile management (e.g. change avatar).
- **Caching & Performance:** Cache AI results (transcripts/analysis) to avoid reprocessing. Implement pagination or indexing on large datasets. Use Firebase Functions to pre-process large files if needed.
- **Real-time Collaboration:** Allow multiple admins to view live copilot suggestions or annotate calls together. 
- **Language Support:** The system claims to handle English, Hindi, Gujarati. Consider adding on-screen language selection or auto-detect.
- **Offline/Local Support:** Currently `localDbService` is deprecated. For offline, consider storing pending uploads or last report in IndexedDB.
- **Test Suite:** Add automated tests (see above) and integrate in CI (GitHub Actions).
- **Documentation:** Expand README with user guide and developer setup. Generate an API specification document from code or swagger (for the /api/gemini).
- **UX Enhancements:** Mobile responsiveness, dark mode, export to PDF/CSV features.

## Deliverables

This analysis can be provided as:
- A **Markdown report** (this document) with embedded code blocks and diagrams.
- A **PDF** version (from the Markdown) for offline reading.
- Supplementary **README.md** updates or developer docs if needed.
- Any required **Diagrams** (as above mermaid diagrams, which can be rendered in docs).
- Example **code snippets** (provided inline).

All relevant code excerpts and references have been drawn from the Callsense repository and official documentation. Unspecified details (e.g. exact error messages, file storage) are noted as such. External sources include Google AI docs and Firebase guides for confirmation of best practices. 

**Sources:** Official Google Gemini API docs and Firebase Security Rules guide were used to understand AI integration and security requirements.  The Callsense code itself (component structure, TypeScript types, Firestore rules) provides the primary details of implementation.
